...
マテリアルの上に配置され、上部のステータスバーとアップデートバーは macOS 26 以降で
Liquid Glass を使います。「透明度を下げる」がオンの場合は、すべて不透明になります。
切り替えインジケーターは別扱いです。ほかのアプリの上に表示されるため、テーマ側で
トーンを固定していない限り、macOS の外観に従います。
